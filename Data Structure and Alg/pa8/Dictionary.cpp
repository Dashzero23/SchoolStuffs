...
void Dictionary::preOrderString(std::string& s, Node* R) const {
    if (R != nil) { // if R is not nil
        s += R->key; // append the node's key to "s"
...
void Dictionary::LeftRotate(Node* x) {
    // set y
    Node* y = x->right; // y = x.right

    // turn y's left subtree into x's right subtree
    x->right = y->left; // x.right = y.left

    if (y->left != nil || y->left->key != "TA said use a very long string that will never appear in the dictionary") { // if y.left != T.nil // not necessary if using sentinal nil node
    ...
    if (x->parent == nil || x->parent->key == "TA said use a very long string that will never appear in the dictionary") { // if x.parent == T.nil
        root = y; // T.root = y
    }
...
void Dictionary::RightRotate(Node* x) {
    // set y
    Node* y = x->left; // y = x.left

    // turn y's right subtree into x's left subtree
    x->left = y->right; // x.left = y.right

    if (y->right != nil || y->right->key != "TA said use a very long string that will never appear in the dictionary") { // if y.right != T.nil // not necessary if using sentinal nil node
...
void Dictionary::RB_InsertFixUp(Node* z) { // closely followed the given pseudo code
    while (z->parent->color == 1) { // while z.parent.color == RED
        if (z->parent == z->parent->parent->left) { // if z.parent == z.parent.parent.left
            Node* y = z->parent->parent->right; // y = z.parent.parent.right

            if (y->color == 1) { // Case 1: if y.color == RED
                z->parent->color = 0; // z.parent.color = BLACK
                y->color = 0; // y.color = BLACK
                z->parent->parent->color = 1; // z.parent.parent.color = RED
                z = z->parent->parent; // z = z.parent.parent
            }
            
            else {
                if (z == z->parent->right) { // Case 2: if z == z.parent.right
                    z = z->parent; // z = z.parent  
                    LeftRotate(z); // LeftRotate(T, z) 
                }

                // Case 3
                z->parent->color = 0; // z.parent.color = BLACK
                z->parent->parent->color = 1; // z.parent.parent.color = RED
                RightRotate(z->parent->parent); // RightRotate(T, z.parent.parent)
            }
        } 
        
        else {
            Node* y = z->parent->parent->left;  // y = z.parent.parent.left

            if (y->color == 1) { // Case 4: if y.color == RED
                z->parent->color = 0; // z.parent.color = BLACK
                y->color = 0; // y.color = BLACK
                z->parent->parent->color = 1; // z.parent.parent.color = RED
                z = z->parent->parent; // z = z.parent.parent
                ...
                if (z == z->parent->left) { // Case 5: if z == z.parent.left
                    z = z->parent; // z = z.parent
                    RightRotate(z); // RightRotate(T, z)
                }

                // Case 6
                z->parent->color = 0; // z.parent.color = BLACK
                z->parent->parent->color = 1; // z.parent.parent.color = RED
                LeftRotate(z->parent->parent); // LeftRotate(T, z.parent.parent)
            }
        }
    }

    root->color = 0; // T.root.color = BLACK
}

void Dictionary::RB_Transplant(Node* u, Node* v){
    if (u->parent == nil || u->parent->key == "TA said use a very long string that will never appear in the dictionary") { // if u.parent == T.nil
        root = v; // T.root = v
    }
...
void Dictionary::setValue(keyType k, valType v) {
    ...
    N->color = 1;
    RB_InsertFixUp(N); // for pa8 add RB_InsertFixUp(N) at this line
}

void Dictionary::remove(keyType k) { // change transplant to RB_Transplant
    ...
    if (N->left == nil || N->left->key == "TA said use a very long string that will never appear in the dictionary") { // if the left node of N is nil:
        RB_Transplant(N, N->right); // call transplant on N and it's right node, decrement num_pairs
        num_pairs--;
    }
        ...
        RB_Transplant(N, N->left); // call transplant on N and it's left node, derement num_pairs
        num_pairs--;
    }
        ...
        if (y->parent != N) { // if the parent node of y is not the same as N:
            RB_Transplant(y, y->right); // call transplant on y and it's right node
            ...
            RB_Transplant(N, y); // call transplant on N and y
